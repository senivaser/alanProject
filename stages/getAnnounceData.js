...
module.exports.getAnnounceData = async (announceId, announceMethod) => {
  ...
  try {
    ...
    const document =  await getDocSafe(env.CD_DATA, config)
    ...
    console.log(err)
    return {
      "Способ": `${announceMethod}`,
      "reqNumber": 999,
      "Номер объявления": `${announceId}`,
      "Наименование объявления": "-",
      "Статус объявления": "-",
      "Дата публикации объявления": new Date(),
      "Срок начала приема заявок": "-",
      "Срок окончания приема заявок": "-", lots: {
      "Ошибка парсера": {
        "spec": "-",
        "№ п/п": "-",
        "Номер лота": "Ошибка парсера",
        "Заказчик": "-",
        "Наименование": "-",
